...
async def plan_trip(request: RouteRequest):
    ...
    try:
        ...
        #     "legs": route_info[0] if isinstance(route_info, list) else route_info
        # })
        # logger.info(f"Weather info received: {weather_info}")
        ...
        # logger.info(f"Fetching recommendations for waypoints: {request.waypoints}")
        ...
        # logger.info(f"Recommendations received: {recommendations}")
        # Get attractions along the route
        route_attractions = await recommendation_agent.get_route_atractions(
            decoded_coordinates,
            request.attraction_preferences
        )

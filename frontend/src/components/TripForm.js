...
const TripForm = ({ onSubmit }) => {
    ...
    const validateForm = () => {
        ...
        // Validate waypoints (only non-empty ones)
            ...
            <div className="form-group">
                ...
                {waypoints.map((waypoint, index) => (
                    <div key={index} className="waypoint-input">
                        <input
                            type="text"
                            placeholder={`Waypoint ${index + 1} (e.g., Mobile, AL)`}
                            value={waypoint}
                        ...
                        {errors[`waypoint${index}`] && (
                            <span className="error-message">{errors[`waypoint${index}`]}</span>
                        )}
                    </div>
                ))}
                <button 
